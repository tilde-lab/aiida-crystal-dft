...
import aiida_crystal
import aiida_crystal.tests.utils as tests
from aiida_crystal.aiida_compatibility import aiida_version, cmp_version, run_get_node, get_calc_log
...
def test_prepare_and_validate(new_database, new_workdir):
    ...
    instruct = DataFactory('structure')(ase=atoms)
    ...
    calc_cls = CalculationFactory('crystal.main')
...
def test_submit_mgo(new_database, new_workdir):
    ...
    inparams = DataFactory('parameter')(dict={
        "title": "MgO Bulk",
        "scf": {
            "k_points": (8, 8)
        }
    ...
    mg_basis, _ = DataFactory('crystal.basisset').get_or_create(
        os.path.join(TEST_DIR, "input_files", "sto3g", 'sto3g_Mg.basis'))
    o_basis, _ = DataFactory('crystal.basisset').get_or_create(
    ...
    with SandboxFolder() as folder:
        subfolder, _ = calc.submit_test(folder=folder)
        ...
        with open(os.path.join(subfolder.abspath,
                               calc._DEFAULT_INPUT_FILE)) as f:  # pylint: disable=protected-access
            input_content = f.read()
        with open(
                os.path.join(subfolder.abspath,
                             calc._DEFAULT_EXTERNAL_FILE)) as f:  # pylint: disable=protected-access
            gui_content = f.read()
...
4
 ...
 28  -2.082000000E+00  -2.082000000E+00   4.440892099E-16
  ...
  8   0.000000000E+00  -2.082000000E+00   4.440892099E-16
123 16
"""
...
def test_parser_with_init_struct(new_database, new_workdir):
    ...
    calc.store_all()
    calc._set_state(calc_states.PARSING)  # pylint: disable=protected-access

    parser_cls = ParserFactory("crystal.basic")
    ...
    with SandboxFolder() as folder:
        ...
        mock_retrieved = {calc._get_linkname_retrieved(): fdata}  # pylint: disable=protected-access
        success, node_list = parser.parse_with_retrieved(mock_retrieved)

    assert success

    node_dict = dict(node_list)
    assert {'output_parameters', 'output_settings'} == set(node_dict.keys())

    expected_params = {
        'parser_version': str(aiida_crystal.__version__),
...
@pytest.mark.skip(reason='Cannot converge in CRYSTAL14')
